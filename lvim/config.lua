  ...
  dap.configurations.cpp = {
    {
      ...
      program = function()
        local path
        local cppfile = vim.fn.expand('%')
        local exefile = string.gsub(cppfile, ".cpp", "")
        vim.ui.input({ prompt = "Path to executable: ", default = vim.loop.cwd() .. "/build/Debug/" .. exefile , completion="file" }, function(input)
          path = input
        end)
        vim.cmd [[redraw]]
        return path
...
--   local filename = vim.expand('%')
--   local extname = string.sub(filename, -2)
--   print(extname)
--   if extname == "py" then
--     vim.cmd [[set fdm=indent]]
--   end
-- end
-- lvim.keys.normal_mode["<Space>t"] = ":lua toggle_python<cr>"

lvim.keys.normal_mode['<Space>zb'] = ":lua require('quizz').build()<cr>"
lvim.keys.normal_mode['<Space>zn'] = ":lua require('quizz').next()<cr>"
lvim.keys.normal_mode['<Space>zp'] = ":lua require('quizz').prev()<cr>"
lvim.keys.normal_mode['<Space>m'] = ":w<cr> :set makeprg=./cmake/utils/compile<cr> :make<cr>"
lvim.keys.normal_mode['<Space>t'] = ":!./cmake/utils/compile -t<cr>"
-- vim.o.makeprg="./cmake/utils/compile -t"
